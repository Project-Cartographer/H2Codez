...
bool __fastcall load_main_window_hook(int thisptr, int unused, int a2, int a3, int a4, int *a5)
{
	...
	HMENU menu = reinterpret_cast<HMENU>(menu_ptr);
...
#pragma region Hooks
	...
	create_main_window_orginal = reinterpret_cast<create_main_window>(0x469030);
	DetourAttach(&(PVOID&)create_main_window_orginal, create_main_window_hook);

	load_main_window_orginal = reinterpret_cast<load_main_window>(0x47ACE0);
